    ...
    <main className="min-h-screen bg-black text-white p-8">
      <div className="mx-auto max-w-6xl space-y-8">
        <div className="flex items-center justify-between gap-4">
          ...
          <Link href="/portal/bookings/new" className="rounded-lg bg-red-600 px-5 py-3 font-semibold">
            New Booking
          </Link>
        </div>
            ...
            {orders?.map((order: any) => (
              <div key={order.id} className="rounded-xl border border-white/10 p-4">
                ...
                {order.stripe_payment_link && (
                  <a href={order.stripe_payment_link} className="mt-2 inline-block text-red-400" target="_blank" rel="noreferrer">
                    Pay now
                  </a>
                )}
              </div>
